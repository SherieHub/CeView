/**
 * Fixture data for Module 2 (Market Radar) — ported verbatim from
 * ui-ux-prototype.html:1093-1220 (`buildChartData`, `MOCK_MARKETS`,
 * `CATEGORY_MARKET_SCORES`, `marketsForCategory`). Used by apiClient's
 * fixture-mode branches so every Market Radar screen is demoable with
 * VITE_USE_FIXTURES=true and no backend running.
 */
...
interface BuildChartDataOptions {
  ...
  gdp: number;
  spikeAt: number | null;
  /** 12 forecast values, index 0 = the "Current" week's forecast, then Wk +1 … Wk +11. */
  weekly: number[];
}

/** Deterministic 24-point series: 12 history ("Wk -11"…"Current") + 12 forecast. */
export function buildChartData(o: BuildChartDataOptions): ChartDataPoint[] {
...
export const MOCK_MARKETS: Market[] = [
  {
    ...
    currency: 'KRW', forexLabel: 'PHP per 1 KRW', gdpValue: 2.2, forexValue: 23.80,
    ...
    chartData: buildChartData({
      base: 52, amp: 9, drift: 1.6, seasonBase: 64, seasonPhase: 0, forex: 23.8, forexAmp: .3, gdp: 2.2, spikeAt: 11,
      weekly: [76.4, 74.9, 73.1, 71.5, 70.2, 69.4, 68.7, 68.1, 67.6, 67.2, 66.8, 66.5],
    }),
  },
  {
    ...
    currency: 'JPY', forexLabel: 'PHP per 1 JPY', gdpValue: 1.4, forexValue: 2.10,
    ...
    chartData: buildChartData({
      base: 44, amp: 7, drift: 1.3, seasonBase: 58, seasonPhase: 2, forex: 2.1, forexAmp: .04, gdp: 1.4, spikeAt: null,
      weekly: [63.8, 65.2, 66.9, 68.4, 69.1, 69.8, 70.3, 70.1, 69.7, 69.2, 68.8, 68.3],
    }),
  },
  {
    ...
    currency: 'USD', forexLabel: 'PHP per 1 USD', gdpValue: 2.5, forexValue: 57.60,
    ...
    chartData: buildChartData({
      base: 38, amp: 6, drift: 1.5, seasonBase: 52, seasonPhase: 4, forex: 57.6, forexAmp: .5, gdp: 2.5, spikeAt: null,
      weekly: [57.2, 58.9, 60.4, 62.1, 63.6, 64.8, 65.9, 66.7, 67.2, 67.5, 67.1, 66.6],
    }),
  },
];
...
export function marketsForCategory(category: string): Market[] {
  ...
  return MOCK_MARKETS
    .map(m => ({ ...m, matchScore: scores[m.id] != null ? scores[m.id] : m.matchScore }))
