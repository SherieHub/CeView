/**
 * Fixture data for Module 3 (ContentResponseDTO / CaptionsByPlatform) —
 * ported verbatim from ui-ux-prototype.html:1278-1391. `ARCHETYPES` is
 * defined earlier in the prototype (line 1093) and ported alongside; the
 * prototype's local `meta(...)` helper (line 1272) is ported as a small
 * typed factory for `CaptionMetadata` entries.
 */
import type { CaptionMetadata, ContentResponseDTO } from '../../types';

/**
 * Demographic-archetype labels parallel to each platform's `options[]` —
 * ui-ux-prototype.html:1093-1097.
 */
...
export const MOCK_CONTENT: ContentResponseDTO = {
  ...
  captions: {
    instagram: {
      ...
      optionMetadata: [
        ...
        meta(
          ...
          "Uses 존댓말-equivalent formal register in English with Hangul category hashtags. Cites the Korean peak-season months explicitly because mature Korean planners plan around school calendars.",
    ...
    facebook: {
      optionNames: ARCHETYPES,
      options: [
        'Booked the 호캉스 yet? 🌴 Sunset Cove sits 30 metres from the Moalboal sardine run — sunrise snorkel, hammock by noon, nothing else on the schedule. Rates and dates → sunsetcove.ph/kr\n\n#세부여행 #호캉스',
