...
| DemandAlert | JPA entity for `tbl_demand_alert`. Created by ForecastingService when spikeIndicator is true. Stores alertLevel (WARNING/CRITICAL), alertMessage, trend, isRead, windowOpenDate, alertDate. FK to tbl_market_score. Read by NotificationService for the notification feed. | JPA entity |
