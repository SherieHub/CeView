...
Submodule 2.2 is the **on-demand** half of Module 2. It is triggered by user actions on the Market Radar view and runs the **Groq `llama-3.3-70b-versatile`** demand forecasting + XGBoost economic scoring pipeline. The forecasting service file is `gemini_forecaster.py` — this name is a historical artifact from the Phase 2 architectural pivot (BiLSTM → AI API); the runtime engine is Groq. Contrast with Submodule 2.1, which runs on scheduled cron jobs (PyTrends ingestion, seasonality detection) and feeds the signal records that 2.2 reads.

| Diagram                 | File                                                                         | Scope                                                                                                                 |
| ----------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Class                   | [`class.puml`](class.puml)                                                   | Spring Boot services, entities, repositories, DTOs + FastAPI forecasting router and service modules (backend only)    |
| ERD                     | [`er.puml`](er.puml)                                                         | 4 primary 2.2 tables + 2 referenced 2.1/anchor tables                                                                 |
| Sequence                | [`sequence.puml`](sequence.puml)                                             | End-to-end flows for GET /markets (DB read) and POST /analyze (full AI pipeline), including frontend rendering impact |
| Transformer Integration | [`../TRANSFORMER_MODEL_INTEGRATION.md`](../TRANSFORMER_MODEL_INTEGRATION.md) | Hugging Face Spaces Transformer Demand Prediction Model (`JamJamzz/ceview-demand-prediction-model`)                   |
...
### FastAPI SBERT (`backend/fastapi-sbert/`)

| Layer             | Component                  | File                                                                                                               | Responsibility                                                                                                                                                                                                                                                 |
| ----------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Router**        | `demand_forecast`          | `app/routers/demand_forecast.py`                                                                                   | Exposes `/internal/forecasting/forecast/series`, `/internal/forecasting/forecast`, and `/internal/forecasting/status`; asynchronous Weights & Biases telemetry                                                                                                 |
| **Service**       | `demand_forecast_service`  | `app/services/demand_forecast_service.py`                                                                          | Direct integration with Hugging Face Space `JamJamzz/ceview-demand-prediction-model`; auto-detects current calendar week; encodes 52-week sinusoidal tensor `[trend_scaled, week_sin, week_cos]`                                                               |
| **Model / DTO**   | `DemandForecastInputClass` | `app/model/DemandForecastInputClass.py`                                                                            | Pydantic schemas for forecasting request & response structures (`DemandForecastInputClass`, `DemandForecastOutputClass`)                                                                                                                                       |
| **Model Service** | `Hugging Face Spaces`      | [`JamJamzz/ceview-demand-prediction-model`](https://huggingface.co/spaces/JamJamzz/ceview-demand-prediction-model) | Transformer-based weekly demand forecasting endpoint (`/forecast` via `gradio_client`); consumes 52-week history, market, and category; yields 12-week forecast trajectory (see [`../TRANSFORMER_MODEL_INTEGRATION.md`](../TRANSFORMER_MODEL_INTEGRATION.md)). |
...
| `StrategicDirectivePanel` | `components/StrategicDirectivePanel.tsx`                          | `market.directive`                                                                                                | Navy panel with Groq `llama-3.3-70b-versatile`-generated directive text; "Generate Content" CTA → navigates to Module 3 Content Studio              |
