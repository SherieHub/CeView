# Lane C — Delete the Frontend Fixture Layer

> **REQUIRED SUB-SKILL:** superpowers:test-driven-development for every code step below.

**Goal:** `services/fixtures/`, the `VITE_USE_FIXTURES` flag, and every
ternary in `apiClient.ts` are gone. Screens with a real backend already
fetch it; screens without one say so via `<NotImplementedPanel>`.

**Prerequisite: [`01-trunk.md`](01-trunk.md) must be merged before you start.**
Task 31 below copies `obDraft.tsx`'s dev-seed object "minus the naver key...
removed [by the trunk]" — this is a real dependency on the trunk's result,
not just a file-touch collision to avoid. Do not start this lane until
you've confirmed the trunk's finished-state checklist is green and merged.

**Touches:** `frontend/services/fixtures/`, `frontend/services/apiClient.ts`,
`frontend/components/shared/NotImplementedPanel.tsx` (new),
`frontend/components/module-1/onboarding/devSeed.ts` (new),
`frontend/components/module-3/3.1-content-studio/omcsRubric.ts` (new), the
three no-backend screens, and `IngestionForm.tsx`. No other lane touches
any of these — except `frontend/types.ts`, see the index's merge-order note
(this lane only deletes `WorkspaceMemberFixture` there, a different region
from what Lane A and the trunk touch).
...
This lane's own verification needs a live backend for its contract-test
step (below) — bring the stack up from `backend/`:
...
`npx tsc --noEmit` has one pre-existing, unrelated error in
`CalendarView.tsx` (`GOLD` token) — confirm you introduce no *new* ones,
don't fix that one. `npm run test:contract` needs the Docker stack up and
auto-skips cleanly if it isn't — don't be alarmed by a skip, but do run it
against a live stack at least once before calling this lane done.
...
- [ ] `grep -rn "VITE_USE_FIXTURES\|services/fixtures" frontend/` — no output
- [ ] `ls frontend/services/fixtures` — fails, directory gone
- [ ] Every screen in all four modules shows real data, a staleness banner, an error panel, or a "Not built yet" panel — nothing else
- [ ] `git log` shows this lane's commits on top of the trunk's merge
