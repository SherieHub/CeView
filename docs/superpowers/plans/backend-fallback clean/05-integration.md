...
**Goal:** the guard script, CI wiring, negative-path tests, provenance e2e,
and docs that prove the whole effort actually holds — and keeps holding
after everyone stops watching it.

**Prerequisite: all three lanes merged.** [`01-trunk.md`](01-trunk.md),
[`02-lane-a-forecasting.md`](02-lane-a-forecasting.md),
[`03-lane-b-content-and-reports.md`](03-lane-b-content-and-reports.md), and
[`04-lane-c-fixtures.md`](04-lane-c-fixtures.md) must all be green and
merged before starting here. This is not parallel with the three lanes —
the guard script's banned-identifier list and the e2e spec's canary strings
only mean anything once the things they check for are actually deleted.
Confirm the merge:

```bash
git log --oneline --all | grep -iE "naver|fallback|fixture|provenance" | head -20
```

You should see commits from all three lanes plus the trunk. If you don't,
stop and check with whoever owns the missing lane before continuing —
running this task against an incomplete merge means the guard script's
"clean" result is meaningless (nothing to catch yet) and the e2e spec will
fail for reasons unrelated to what it's actually testing.

**Touches:** `scripts/`, `.github/workflows/*.yml`, `frontend/tests/contract/`,
`e2e/tests/`, `RUNNING.md`, `.claude/CLAUDE.md`. Nothing here overlaps any
lane's files — this task only reads what they deleted, via grep and tests,
...
The full stack must be up for the negative-path contract test (Task 38),
which additionally needs a **second** compose invocation with the LLM
deliberately unconfigured:

```bash
cd backend && docker compose up -d
# then, for Task 38 specifically:
docker compose -f docker-compose.yml -f docker-compose.no-llm.yml up -d
```
