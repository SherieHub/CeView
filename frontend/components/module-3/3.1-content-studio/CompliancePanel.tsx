...
import { MOCK_OMCS, OMCS_RUBRIC_LABELS } from '../../../services/fixtures/omcs';
...
export default function CompliancePanel({ draft, audit, onAuditChange }: ComplianceSlotProps) {
  ...
  useEffect(() => {
    ...
    if (audit.step >= STEPS.length) { onAuditChange({ status: 'complete', step: STEPS.length, result: MOCK_OMCS }); return; }
  ...
  return <section className="card" aria-labelledby="compliance-title">
    ...
    {audit.status === 'running' && <div className="mt-5 space-y-3" aria-live="polite">{STEPS.map((step, index) => <div key={step} className="flex items-center gap-3 text-sm"><span className="grid h-6 w-6 place-items-center rounded-full bg-mint-pale text-teal-accent">{index < audit.step ? <CheckCircle2 size={15} /> : index === audit.step ? <LoaderCircle size={15} className="animate-spin" /> : index + 1}</span><span className={index <= audit.step ? 'text-navy-dark' : 'text-muted'}>{step}</span></div>)}</div>}
    {result && <div className="mt-5"><div className="flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-full border-8 border-success text-xl font-bold text-navy-dark">{Math.round(result.omcsScore)}</div><div><p className="inline-flex items-center gap-1 rounded-full bg-mint-pale px-3 py-1 text-sm font-semibold text-success"><CheckCircle2 size={15} />{result.status}</p><p className="mt-2 text-sm text-muted">OMCS score (pass threshold: 70)</p></div></div><p className="mt-4 rounded-lg bg-mint-pale p-3 text-sm leading-6 text-navy-dark">{result.feedback}</p><div className="mt-4 space-y-2">{Object.entries(result.rubricEvaluationData.scores).map(([key, value]) => <div key={key}><div className="flex justify-between gap-4 text-xs text-muted"><span>{OMCS_RUBRIC_LABELS[key]}</span><span>{value}/100</span></div><div className="mt-1 h-2 overflow-hidden rounded-full bg-gray-light"><div className="h-full bg-teal-accent" style={{ width: `${value}%` }} /></div></div>)}</div><p className="mt-4 text-sm leading-6 text-muted">{result.consistencyExplanation}</p><button type="button" className="btn btn--secondary mt-4" onClick={() => onAuditChange({ status: 'idle', step: 0, result: null })}><RefreshCw size={16} />Re-run audit</button></div>}
  </section>;
}
