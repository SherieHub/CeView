...
export default function CompliancePanel({ draft, audit, onAuditChange }: ComplianceSlotProps) {
  ...
  return <section className="card" aria-labelledby="compliance-title">
    <PanelHead
      ...
      subtitle="Checks the caption and selected media before publishing."
    />
    {audit.status === 'idle' && <p className="mt-4 rounded-lg bg-mint-pale p-4 text-sm leading-6 text-navy-dark">Add a caption, media, and a publish destination, then confirm authorisation to begin the six-step audit.</p>}
    ...
    {audit.status === 'error' && (
      <div className="mt-5">
        ...
        <button type="button" className="btn-outline btn-outline--sm mt-4" onClick={() => onAuditChange({ status: 'idle', step: 0, result: null })}><RefreshCw size={15} />Re-run audit</button>
      </div>
    )}
    {result && <div className="mt-5"><div className="flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-full border-8 border-success text-xl font-bold text-navy-dark">{Math.round(result.omcsScore)}</div><div><p className="inline-flex items-center gap-1 rounded-full bg-mint-pale px-3 py-1 text-sm font-semibold text-success"><CheckCircle2 size={15} />{result.status}</p><p className="mt-2 text-sm text-[var(--color-text-muted)]">OMCS score (pass threshold: 70)</p></div></div><p className="mt-4 rounded-lg bg-mint-pale p-3 text-sm leading-6 text-navy-dark">{result.feedback}</p><div className="mt-4 space-y-2">{Object.entries(result.rubricEvaluationData.scores).map(([key, value]) => <div key={key}><div className="flex justify-between gap-4 text-xs text-[var(--color-text-muted)]"><span>{OMCS_RUBRIC_LABELS[key]}</span><span>{value}/100</span></div><div className="mt-1 h-2 overflow-hidden rounded-full bg-gray-light"><div className="h-full bg-teal-accent" style={{ width: `${value}%` }} /></div></div>)}</div><p className="mt-4 text-sm leading-6 text-[var(--color-text-muted)]">{result.consistencyExplanation}</p><button type="button" className="btn-outline btn-outline--sm mt-4" onClick={() => onAuditChange({ status: 'idle', step: 0, result: null })}><RefreshCw size={15} />Re-run audit</button></div>}
  </section>;
}
