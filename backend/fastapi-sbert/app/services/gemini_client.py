...
if GEMINI_API_KEY:
    try:
        ...
        # Using gemini-1.5-flash as it is highly optimized for fast, structured JSON output
...
def _generate_json(prompt: str, schema: dict | None = None) -> dict:
    ...
    try:
        resp = _client.generate_content(
            prompt,
            # Enforce strict JSON output from the Gemini model
...
# ── Module 4 — PES Compute AI Insights ─────────────────────────────────────

_pes_log = logging.getLogger("module4.pes_compute.ai")
...
def pes_compute_insights(
    ...
    """
    Generate AI prescriptive insights for a freshly computed PES result (UC-4.3).

    Prompt instructs the LLM to:
      1. Identify the weakest funnel stage based on metric drop-offs
      2. Rank 3 actionable optimisation recommendations for Cebu MSMEs
      3. Write a plain-language executive summary
    ...
    if not _enabled():
        _pes_log.info("AI disabled — pes_compute_insights returning fallback")
        return fallback

    breakdown_str = "\n".join(
        f"  • {b['metric']}  (base weight {b['weight']}, contribution = {b['contribution']:.4f})"
        for b in breakdown
    )
    flagged_str   = ", ".join(flagged_metrics) if flagged_metrics else "None"

    prompt = f"""You are CeView's Senior Campaign Analyst, specialising in digital marketing
optimisation for Cebu, Philippines tourism MSMEs.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
COMPUTED PES ANALYSIS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Overall PES Score : {pes_score:.4f} / 1.00  →  {pes_label}

Metric Contribution Breakdown:
{breakdown_str}

Raw Computed Metrics:
  CTR  = {base_metrics.get('CTR',  0):.4f} %
  CPC  = ₱{base_metrics.get('CPC',  0):.4f}
  CR   = {base_metrics.get('CR',   0):.4f} %
  ROAS = {base_metrics.get('ROAS', 0):.4f} ×
  CAC  = ₱{base_metrics.get('CAC',  0):.4f}

Flagged / Excluded Metrics (zero values): {flagged_str}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FUNNEL STAGES (reference)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Stage 1: Impressions → Clicks     (measured by CTR, CPC)
Stage 2: Clicks → Bookings        (measured by Conv. Rate / CR)
Stage 3: Bookings → New Customers (measured by CAC)
Stage 4: Spend → Revenue          (measured by ROAS)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TASKS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Identify the WEAKEST funnel stage based on which metric(s) contribute the least
   to the PES score above. Choose one of the four stage labels.
2. Generate EXACTLY 3 ranked, actionable optimisation recommendations.
   Each must be a SPECIFIC, concrete action sentence tailored to a Cebu MSME
   hospitality / tourism operator — not generic marketing advice.
   Rank 1 = highest business impact.
3. Write a concise executive summary (2–3 sentences) that interprets the PES
   score, names the primary bottleneck, and states the expected benefit of fixing it.

Return ONLY valid JSON (no markdown, no explanation) with this exact structure:
{{
  "weakest_funnel_stage": "<one of the four stage labels above>",
  "recommendations": [
    "<Rank-1 specific action>",
    "<Rank-2 specific action>",
    "<Rank-3 specific action>"
  ],
  "executive_summary": "<2–3 sentences>"
    ...
    try:
        ...
        _pes_log.warning("pes_compute_insights AI call failed: %s", exc)
        return fallback

    if not out or "recommendations" not in out:
        _pes_log.warning("pes_compute_insights: empty AI response — returning fallback")
        return fallback

    recs = list(out.get("recommendations") or [])[:3]
    # Pad to exactly 3 items if the LLM returned fewer
    while len(recs) < 3:
        recs.append("Consult your marketing team for a further review of this metric.")

    _pes_log.info("pes_compute_insights ok  weakest=%s", out.get("weakest_funnel_stage"))
    return {
        "weakest_funnel_stage": str(out.get("weakest_funnel_stage", "Clicks → Bookings")),
        "recommendations":      recs,
        "executive_summary":    str(out.get("executive_summary", "")),
        "source":               "gemini",
    }
